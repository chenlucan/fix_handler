...
#ifndef __FH_TMALPHA_REPLAY_MOCK_REPLAY_DATA_PROVIDER_H__
#define __FH_TMALPHA_REPLAY_MOCK_REPLAY_DATA_PROVIDER_H__
...
namespace replay
{
    class MockReplayDataProvider : public fh::tmalpha::market::DataProvider
    {
        public:
            MockReplayDataProvider(const std::vector<std::string> &messages, std::uint32_t page_size)
            : m_messages(messages), m_page_size(page_size)
            {
            }

            virtual ~MockReplayDataProvider()
            {
            }
        ...
        private:
            DISALLOW_COPY_AND_ASSIGN(MockReplayDataProvider);
    };
}   // namespace market
}   // namespace tmalpha
}   // namespace fh

#endif  // __FH_TMALPHA_REPLAY_MOCK_REPLAY_DATA_PROVIDER_H__
