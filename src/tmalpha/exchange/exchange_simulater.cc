...
namespace exchange
{
    ...
    // 新订单来的时候看看和当前行情能否成交
    bool ExchangeSimulater::Has_matching(const ::pb::ems::Order& org_order) const
    {
    ...
    {
        const pb::ems::Order &org_order = std::get<0>(order_info);
            ...
            std::function<bool(double, double)> compare) const
    {
        ...
        if(compare(best_price, order_price)) return false;
        // 订单的报价优于当前行情的最优报价，该订单马上可以成交
        if(compare(order_price, best_price)) return true;
    ...
    bool ExchangeSimulater::Is_price_matching(const ::pb::ems::Order& org_order, const pb::dms::L2 &l2)
    {
        if(org_order.buy_sell() == pb::ems::BuySell::BS_Buy)
        {
            // 没有卖方报价，不能成交
            ...
            // 订单是 market order 的话（没指定价格），马上成交
            if(org_order.order_type() == pb::ems::OrderType::OT_Market) return true;

            // 可以马上成交的条件为：报单价格匹配到当前对手方报价
            return std::stod(org_order.price()) >= l2.offer(0).price();
        }
        else if(org_order.buy_sell() == pb::ems::BuySell::BS_Sell)
        {
            // 没有买方报价，不能成交
            ...
            // 可以马上成交的条件为：报单价格匹配到当前对手方报价
            return std::stod(org_order.price()) <= l2.bid(0).price();
        }
