...
namespace market
{
    ...
    DatProcessor::DatProcessor(fh::cme::market::CmeData *data, fh::cme::market::DatReplayer *replayer, std::function<void(void)> on_error)
    : m_arbitrator(), m_pData(data), m_replayer(replayer), m_on_error(on_error)
    {
        // noop
    }
    ...
    void DatProcessor::Process_replay_data(char *buffer, const size_t data_length)
    {
        ...
        LOG_DEBUG("***** [DatProcessor::Process_replay_data] (status=", status, ", packet_seq_num=", packet_seq_num, ", std::numeric_limits<std::uint32_t>::max()=", std::numeric_limits<std::uint32_t>::max(), " *****");
    ...
    void DatProcessor::Process_feed_data(char *buffer, const size_t data_length)
    {
        ...
        LOG_DEBUG("***** [DatProcessor::Process_feed_data] (status=", status, ", seq=", packet_seq_num, ") *****");
    ...
    {
        LOG_DEBUG("***** [DatProcessor::Process_data]  *****");
    ...
    {
        LOG_DEBUG("***** [DatProcessor::Start_tcp_replay]  *****");
    ...
    {
		m_pData->Insert_increment_data(packet_seq_num, mdp_messages);
    }
