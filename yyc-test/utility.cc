...
namespace utility
{
    ...
    std::string Calculate_fix_checksum(const std::string &message)
    {
    	...
    	char checksum[8];
    	sprintf(checksum, "10=%03d\001", sum % 256);
    ...
    std::string Make_fix_logon_message(const std::string &username, const std::string &password)
    {
    	...
    	std::string checksum = Calculate_fix_checksum(message);
    ...
    {
        auto index = rczg::utility::Random_number(0, len - 1);
        ...
        LOG_DEBUG("message: size=", message_size, " msg=", rczg::utility::Hex_str(encoded.first, message_size));
