...
class TCPSession: public std::enable_shared_from_this<TCPSession>
{
	...
	private:
		...
		{
			std::string message(data, length);
			LOG_INFO("tcp sent: length=", length, " type=", message_type, " seq=", current_seq, " data=", rczg::utility::Hex_str(data, length));
		...
		{
			uint32_t ids[] = {15};
			std::size_t length = rczg::utility::Make_packet(m_data + 2, 0, ids, 1);
		...
		{
            uint32_t ids[] = {30, 37};
            std::size_t length = rczg::utility::Make_packet(m_data + 2, seq, ids, 2);
            memcpy(m_data, (char *)&length, 2);	// first 2 bytes is packet length
            return length + 2;
		}
